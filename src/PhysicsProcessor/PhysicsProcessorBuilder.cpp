...
IPhysicsProcessor* PhysicsProcessorBuilder::build(KernelBuilder& kernelBuilder, StructCollector& structCollector, uint32_t texture, engineConfig config, uint platformID, uint deviceID){
    ...
    defaultPlatform.getInfo(CL_PLATFORM_NAME, &platformName);
    fprintf(stdout, "Default Platform:\n\tName: %s\n", platformName.c_str());

    std::string deviceName;
    defaultDevice.getInfo(CL_DEVICE_NAME, &deviceName);
...
#ifdef __APPLE__
    const char * extension_name = ("cl_APPLE_gl_sharing");
#else
    const char * extension_name = ("cl_khr_gl_sharing");
#endif

    size_t found = platformExtensions.find(extension_name);

    if (found == std::string::npos) {
        fprintf(stderr, "Platform does not support %s\n", extension_name);
        return nullptr;
    }
    ...
    if (buildCode == CL_SUCCESS){
        ...
        return new PhysicsProcessor(context, TACHYON_ENGINE, texture, config, defaultDevice);
    }
    ...
    if (buildCode == CL_SUCCESS) {
        ...
        cl::Kernel TACHYON_ENGINE(program, "TACHYON_ENGINE");
        return new PhysicsProcessor_Fallback(context, TACHYON_ENGINE, texture, config, defaultDevice);
    }
